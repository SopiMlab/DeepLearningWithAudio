...
class gansynth(pyext._class):
    ...
    def load_1(self, ckpt_dir, batch_size=1):
        ...
        gen_script = os.path.join(os.path.dirname(os.path.realpath(__file__)), "gansynth_child_process.py")
        ckpt_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), str(ckpt_dir))

        print("starting gansynth_child_process process, this may take a while", file=sys.stderr)
        ...
        print("gansynth_child_process is ready", file=sys.stderr)
    ...
    def unload_1(self):
        if self._proc:
            ...
            print("no gansynth_child_process process is running", file=sys.stderr)
    ...
    def _keep_printing_stderr(self):
        while True:
            ...
            sys.stderr.write("[gansynth_child_process] {}".format(line))
            sys.stderr.flush()
