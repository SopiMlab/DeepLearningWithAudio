...
class gansynth_ext(pyext._class):
    def __init__(self, *args):
        self._inlets = 1
        self._outlets = 1
        self._link = None        

    def _load_local(self, ckpt_dir, batch_size=1):
        raise NotImplementedError("_load")

    def _unload(self, *args):
        raise NotImplementedError("_unload")

    def load_1(self, *args):
        loaded = self._load(*args)
        self._outlet(1, ["loaded", *loaded])
        
    def unload_1(self, *args):
        unloaded = self._unload(*args)
        self._outlet(1, ["unloaded", *unloaded])
