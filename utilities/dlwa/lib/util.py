...
def screen_attach(args):
    ...
    runner.run_script(f"attach {name} screen", runner.make_screen_attach_script(name), capture_output=False)

def screen_kill(args):
    runner = common.Runner()
    name = runner.screen_name
    runner.run_script(f"kill {name} screen", runner.make_screen_kill_script(name), capture_output=False)
